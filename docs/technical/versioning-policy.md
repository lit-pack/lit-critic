...
For executing version bumps, use the `versioning` workflow. This document is reference material only.
...
- `core/api.py` FastAPI app `version` binds to the `CORE_VERSION` constant — no direct edit needed when bumping
...
- `api/app.py` FastAPI app `version` binds to the `WEB_VERSION` constant — no direct edit needed when bumping
...
## 6) Work-in-Progress Push Declaration (Non-Release)
...
"release_intent": {
  "status": "wip",
  "bump_type": "patch | feature",
  "components": ["component names affected"],
...
- Before a real release, run `versioning release patch|feature` — this will remove the WIP declaration automatically.
...
## 8) MAJOR Version Bumps

MAJOR bumps (breaking changes) are not handled by the `versioning` workflow. When a MAJOR bump is needed:

1. Bump the component version manually in `versioning/compatibility.json`
2. Update all version declaration files for that component (see section 4)
3. Update the `compatibility` matrix majors in `versioning/compatibility.json` for any consumers of the changed component
4. Run `python versioning/validate_semver.py`
